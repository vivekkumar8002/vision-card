...
function ProductPage() {
  ...
  useEffect(() => {
    document.title = `${product.title.toUpperCase()} | vivek Eyewear Online Store`;
  }, [product.title]);
  ...
  return (
    <>
      {/* container holds product details and sidebar instructions */}
      <div className="pd-container">
        <main className="pd-pg">
          ...
          <div className="pd-pg__img--wrapper">
            {/* Show 360-degree viewer for sunglasses with 360 view available */}
            {product.type === 'sunnies' && product.has360View && product.images['360'] ? (
              <div className="pd-pg__360-viewer">
                <ThreeSixtyViewer 
                  images={product.images['360']} 
                  autoRotate={false}
                  rotationSpeed={1}
                />
              </div>
            ) : (
              /* Default image view for eyeglasses or products without 360 */
              <ImageWrapper>
                {activeImg === 1 ? (
                  <img
            ...
            {/* Only show thumbnail navigation for non-360 view */}
            {(!product.has360View || product.type !== 'sunnies') && (
        ...
        {/* sidebar containing prescription instructions displayed next to product */}
        <aside className="pd-lens">
          <ScrollToTop />
        </aside>
      </div>
    </>
